    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav class="flex mb-6" aria-label="Breadcrumb">
            <ol class="flex items-center space-x-2">
                ...
                <li class="flex items-center">
                    ...
                    <a href="{{ route('plans.show', ['type' => $type, 'id' => $entity->id]) }}" class="ml-2 text-sm font-medium text-gray-500 dark:text-gray-400 hover:text-gray-700">{{ $entity->name }}</a>
                </li>
                <li class="flex items-center">
                    <svg class="h-5 w-5 text-gray-400" viewBox="0 0 20 20" fill="currentColor">
                        <path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"/>
                    </svg>
                    <span class="ml-2 text-sm font-bold text-gray-900 dark:text-white">Add New Chapter</span>
                </li>
            </ol>
        </nav>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
            <div class="p-8">
                ...
                <div class="mb-6">
                    <h2 class="text-2xl font-black text-gray-900 dark:text-white tracking-tight">Add Plan Chapter</h2>
                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Insert a new section or chapter into the pandemic preparedness plan for {{ $entity->name }}.</p>
                </div>

                <form action="{{ route('plans.sections.store', ['type' => $type, 'id' => $entity->id]) }}" method="POST">
                    @csrf
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="content" id="content" rows="12" required
                                  ...
                                  placeholder="<p class='mb-4'>Add paragraph text here...</p>">{{ old('content') }}</textarea>
                        @error('content')
                            <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex justify-end space-x-3 border-t border-gray-100 dark:border-gray-700 pt-6">
                        <a href="{{ route('plans.show', ['type' => $type, 'id' => $entity->id]) }}" 
